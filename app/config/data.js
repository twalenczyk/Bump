export const posts = [
    {'name': 'Ben', 'content': 'Sup', 'id': 1},
    {'name': 'Sarah', 'content': 'Hi', 'id': 2},
    {'name': 'Robert', 'content': 'How do you do?', 'id': 3},
    {'name': 'Mary', 'content': 'Not bad', 'id': 4},
    {'name': 'Daniel', 'content': 'How about yourself?', 'id': 5},
    {'name': 'Laura', 'content': 'Not too shabby, amigo.', 'id': 6},
    {'name': 'John', 'content': 'I cannot wait to get home and have a beer', 'id': 7},
    {'name': 'Debra', 'content': 'Nothing better than a cold one!', 'id': 8},
    {'name': 'Aron', 'content': 'Right?', 'id': 9},
    {'name': 'Ann', 'content': 'It is like we are the smae person', 'id': 10},
    {'name': 'Steve', 'content': 'Wait', 'id': 11},
    {'name': 'Olivia', 'content': 'We are.', 'id': 12},
    {'name': 'Ben', 'content': 'Sup', 'id': 13},
    {'name': 'Sarah', 'content': 'Hi', 'id': 14},
    {'name': 'Robert', 'content': 'How do you do?', 'id': 15},
    {'name': 'Mary', 'content': 'Not bad', 'id': 16},
    {'name': 'Daniel', 'content': 'How about yourself?', 'id': 17},
    {'name': 'Laura', 'content': 'Not too shabby, amigo.', 'id': 18},
    {'name': 'John', 'content': 'I cannot wait to get home and have a beer', 'id': 19},
    {'name': 'Debra', 'content': 'Nothing better than a cold one!', 'id': 20},
    {'name': 'Aron', 'content': 'Right?', 'id': 21},
    {'name': 'Ann', 'content': 'It is like we are the smae person', 'id': 22},
    {'name': 'Steve', 'content': 'Wait', 'id': 23},
    {'name': 'Olivia', 'content': 'We are.', 'id': 24}
]

export const users = [
    {name: 'Taylor Walenczyk', password: 'admin', id: 0},
    {name: 'Bill Burr', password: 'Burr', id: 1},
    {name: 'Armaan', password: 'admin', id: 2},
    {name: 'Jack', password: 'admin', id: 3}
]

export const comments = [
    {id: 0, name: 'Bill', content: 'This sucks!', editable: true},
    {id: 1, name: 'Stan', content: 'I know, right?', editable: false}
]
